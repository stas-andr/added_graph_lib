...
struct Regular_hexagon: Shape{
    ...
    int get_r() const {return radius;}
    Point get_c() const {return center;}
...
struct Arc : Shape {
    Arc(Point p, int ww, int hh, int sg, int eg)
    :w(ww), h(hh), start_g(sg), end_g(eg)
    {
        add(Point{ p.x - ww, p.y - hh });
    }
    void draw_lines() const;
    void set_major(int ww) { w=ww; }
    void set_minor(int hh) { h=hh; }
    void set_start_g(int sg) {start_g=sg;}
    void set_end_g(int eg) {end_g=eg;}
    int major() const { return w; }
    int minor() const { return h; }
    int start_angle() const {return start_g;}
    int end_angle() const {return end_g;}
private:
    int w;
    int h;
    int start_g;
